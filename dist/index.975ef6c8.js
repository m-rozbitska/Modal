...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    fetch("https://jsonplaceholder.typicode.com/users").then((response)=>response.json()).then((isUser)=>{
        if (isUser != null) isUser.forEach((element)=>{
            usersResult.innerHTML += `<div class = "user__html-title" id="userTitleId${element.id}">
    ...
    userBtn.forEach((elem)=>{
        elem.addEventListener("click", function(event) {
            ...
            fetch("https://jsonplaceholder.typicode.com/users/1/todos").then((response)=>response.json()).then((isUser)=>{
                if (isUser[userTodo].id != null) userBtn.closest(".user__html-title").querySelector(".user__info").innerHTML = `<p>User id: ${isUser[userTodo].userId}</p>
                           `;
                else return;
            });
        });
